...
OPENROUTER_API_KEY = "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"  # Replace with your actual API key
...
def insert_into_database(track_id, speed, timestamp, model, color, company, number_plate):
    """Insert detected vehicle details into MySQL database with violation column."""
    try:
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor()

        violation = "yes" if speed > 50 else "no"

        query = """
        INSERT INTO vehicle_records (track_id, speed, date_time, vehicle_model, vehicle_color, vehicle_company, number_plate, violation)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """
        values = (track_id, speed, timestamp, model, color, company, number_plate, violation)

        cursor.execute(query, values)
        conn.commit()
        cursor.close()
        conn.close()
        print(f"✅ Data inserted for Track ID: {track_id} (Violation: {violation})")
    except Exception as e:
        ...
        for box, track_id, cls in zip(self.boxes, self.track_ids, self.clss):
            ...
            if direction == "known" and track_id not in self.trkd_ids:
                ...
                if time_difference > 0:
                                base_speed = 30 
                measured_speed = np.linalg.norm(np.array(curr_pos[:2]) - np.array(prev_pos[:2])) / time_difference
                total_speed = round(measured_speed + base_speed)
                self.spd[track_id] = total_speed
            ...
            if speed_value > 50 and track_id not in self.saved_ids:
